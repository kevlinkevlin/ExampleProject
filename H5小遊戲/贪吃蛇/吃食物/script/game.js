...
var class_game = {
    ...
    _draw : function () {
        ...
        this._food_id = this._food_x*this._column + this._food_y;                       //我们得到食物的ID，方便下面的计算
        //然后我们在地图上标注，我们规定 1 为蛇 2 为食物 0 为地面
        this._map[this._snake_x][this._snake_y] = 1;
        this._map[this._food_x][this._food_y] = 2;
        this._map_id[this._snake] = 1;
        this._map_id[this._food_id] = 2;
        for (let i = 0 ; i < this._row ; i++){
            for (let j = 0 ; j < this._column ; j++){
    ...
    _move : function () {
        ...
        //console.log(_snake_head);
        this._map_id[_snake_head] = 1;           //在地图上标记
        //如果已经移动了的话，我们就每次让尾巴去掉就行
        if(this._direction !== null){
            if(_snake_head !== this._food_id){
                class_base._get(this._snake[this._snake.length-1]).setAttribute("class","box");   //将最后一个变成地图
                this._snake.pop();      //去掉最后一位
            }
            else {
                do {
                    this._food_id = Math.floor(Math.random()*this._row*this._column);      //如果食物的ID等于蛇头，我们就重新弄一个食物
                }while (this._map_id[this._food_id] !== 0);

                class_base._get(this._food_id).setAttribute("class","food");
            }
        }
        if(this._direction === null){
            this._snake.pop();
        ...
        for(let i = 1 ; i < this._snake.length ; i++){
            class_base._get(this._snake[i]).setAttribute("class","snake");    //让蛇显示出来
        }
        class_base._get(this._snake[0]).setAttribute("class","snakehead");
...
var class_base = {
    ...
    changeX : function (_num , _column) {
        return _num/_column;
    },
    changeY : function (_num , _column) {
        return _num%_column;
    }
};